...
def train_siamese_network(train_dataloader,
                          ...
                          embedding_dim=512,
                          fine_tune_last_layers=True,   # New flag to control fine tuning last layers only.
    ...
    Parameters:
      ...
      fine_tune_last_layers (bool): If True, freeze all layers except the last layers of the backbone.
    ...
    # Initialize the backbone and Siamese network.
    ...
    if fine_tune_last_layers:
        ...
        # Unfreeze parameters in the last layers only.
        for name, param in backbone.model.named_parameters():
            if 'block8' in name or 'avgpool_1a' in name or 'last_linear' in name:
                param.requires_grad = True
        # Also unfreeze the additional embedding layer if it exists.
        if backbone.embedding_layer is not None:
            for param in backbone.embedding_layer.parameters():
                param.requires_grad = True
        print("Fine tuning last layers only: Backbone's initial layers frozen.")
    else:
        print("Training the entire backbone (no freezing applied).")
    
    model = SiameseNetwork(base_net=backbone)
    model = model.to(device)

    # Only update parameters that are trainable.
...
if __name__ == '__main__':
    ...
    parser.add_argument("--fine_tune_last_layers", type=bool, default=True, help="If True, only fine tune the last layers of the backbone.")
    parser.add_argument("--run_dir", type=str, default="/home/seif_elkerdany/projects/modeling/model/runs/siamese_B3_net", help="TensorBoard log directory.")
    parser.add_argument("--checkpoint_dir", type=str, default="/home/seif_elkerdany/projects/modeling/model/checkpoints/B3", help="Directory to save model checkpoints and plots.")
